...
# and save it
write.csv(combined_feature_df, file = paste0(datasets, "syllables_soundgen_summary.csv"), row.names = FALSE)
...
data_prepost <- data %>%
  ...
  mutate(annotationNumTarget = annotationNum + 1) %>%
  # Join data with itself based on relevant columns
  left_join(data, by = c("language", "participant", "itemType", "focus", "itemNum", "annotationNumTarget" = "annotationNum"), suffix = c("", ".post")) %>%
