...
```{r loop and chop syllTextable, echo=TRUE, message=FALSE, warning=FALSE}
# Loop through .wav files
for (wavFile in wavFiles) {
  ...
  if (file.exists(textGridFile)) {
    ...
    for (i in 1:nrow(tiersyllText)) {
      ...
      syllTextText <- sanitize_filename(intervalsyllText$text)
      syllTextText <- str_replace_all(syllTextText, "_", "") # Remove underscores because they will disturb our categorization later on
      annotationNum <- intervalsyllText$annotation_num
      
      # Proceed only if syllTextText is not empty
      if (syllTextText != "") {
...
rm(startTime, endTime, outputFile, tg, i, tiersyllText, tierWord, tierProm, wordMatch, promMatch, intervalsyllText, audio, wordText, promText, wordTier, promTier, syllTextText, targetTier, annotationNum, wavFile, wavFiles, textGridFile)
```
...
colnames(fileDurationsDF) <- c("fileName", "duration", "language", "participant", "itemType", 
                               "itemNum", "focus", "annotationNum", "word", "syllTextText", "percProm")

# Reorder columns
fileDurationsDF <- fileDurationsDF[, c("fileName", "language", "participant", "itemType", 
                                       "itemNum", "focus", "annotationNum", "word", "syllTextText", 
                                       "percProm", "duration")]
...
5.  Spectral Features:
    ...
    Spectral centroid and entropy describe the distribution of energy in
    the speech spectrum and correlate with stress and clarity.
...
-   **Amplitude**: `ampl_median`, `ampl_noSilence_median`, `env_slope`

-   **Pitch**: `pitch_median`, `pitch_sd`,`f0_slope`

-   **Formants**: `f1_freq_median`, `f2_freq_median`

-   **Spectral**: `specCentroid_median`, `entropy_median`

-   **Voice Quality**: `HNR_median`

-   **Temporal**: `amEnvDep_median`, `fmDep_median`
...
We need to identify the corresponding pre- and post-tonic syllables, because we assume that prosody is a function of the environment.
...
```{r save final data, echo=TRUE, message=FALSE, warning=FALSE}
